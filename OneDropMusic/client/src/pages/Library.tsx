...
interface ProcessedVideo {
  ...
  status: string;
  processedAt: string;
  bpm?: number | null;           // ← FIX 1: add bpm to backend response type
}
...
interface LibraryTrack {
  ...
  addedAt: string;
  status: string;
  bpm?: number | null;           // already present
}
...
export default function Library() {
    ...
    const fetchLibrary = async () => {
      ...
      try {
        ...
        const formattedLibrary: LibraryTrack[] = data.map((video) => ({
          ...
          status: video.status,
          bpm: video.bpm ?? null,      // ← FIX 2: map bpm from API response
        }));
  ...
  const handleOpenStudio = (track: LibraryTrack) => {
    ...
    const studioData = {
      ...
      bpm: track.bpm ?? null,          // ← already present, now has real value
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      {/* ← FIX 3: show BPM badge in library if available */}
                      {track.bpm && (
                        <span className="px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold tabular-nums">
                          {track.bpm} BPM
                        </span>
                      )}
                    </div>
