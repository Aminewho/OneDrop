...
const Card = ({ children, className }: CardProps) => (
    <div className={`rounded-2xl border border-white/5 bg-[#141416] text-foreground shadow-2xl ${className}`}>
        {children}
    </div>
);
...
const Button = ({ children, onClick, className, variant = "default", disabled = false, size = "default" }: ButtonProps) => {
    ...
    switch (variant) {
        case 'default':
            ...
            break;
        case 'outline':
            variantStyles = 'bg-transparent border border-white/10 text-muted-foreground hover:bg-card/5 hover:border-white/20';
            break;
        case 'secondary':
            variantStyles = 'bg-card/5 text-muted-foreground hover:bg-card/10 border border-white/5';
            break;
        case 'danger':
            variantStyles = 'bg-destructive/20 text-destructive hover:bg-destructive/30 border border-destructive/30';
            break;
        case 'ghost':
            variantStyles = 'bg-transparent text-muted-foreground hover:text-primary-foreground hover:bg-card/5';
            break;
        default:
...
const STEM_CONFIG: Record<string, { color: string; bg: string; icon: ReactNode; label: string }> = {
    vocals:  { color: 'hsl(var(--destructive))', bg: 'rgba(248,113,113,0.08)', icon: <Music  className="w-4 h-4" />, label: 'Vocals'  },
    drums:   { color: 'hsl(var(--primary))', bg: 'rgba(96,165,250,0.08)',  icon: <Disc3  className="w-4 h-4" />, label: 'Drums'   },
    drum:    { color: 'hsl(var(--primary))', bg: 'rgba(96,165,250,0.08)',  icon: <Disc3  className="w-4 h-4" />, label: 'Drums'   },
    bass:    { color: 'hsl(var(--accent))', bg: 'rgba(52,211,153,0.08)',  icon: <Aperture className="w-4 h-4" />, label: 'Bass' },
    other:   { color: 'hsl(var(--secondary))', bg: 'rgba(251,191,36,0.08)',  icon: <Zap    className="w-4 h-4" />, label: 'Other'   },
};
...
export default function Separator() {
    ...
    if (!studioData) {
        return <div className="p-6 text-center text-destructive min-h-screen bg-[#0c0c0e] flex items-center justify-center">Missing studio data. Return to library.</div>;
    }
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-8">
                    <Button
                        ...
                        className="text-muted-foreground hover:text-primary-foreground gap-2"
                    >
                    ...
                    <div className="text-center">
                        ...
                        <h1 className="text-lg font-semibold text-primary-foreground truncate max-w-[260px]" title={studioData.trackTitle}>
                ...
                <div className="mb-4 p-5 rounded-2xl bg-[#141416] border border-white/5 shadow-2xl">
                    ...
                    <div className="flex items-center gap-3 mb-5">
                        <span className="text-xs font-mono text-muted-foreground w-10 text-right shrink-0 tabular-nums">
                            {formatTime(playbackTime)}
                        </span>

                        <div className="flex-1 relative h-1.5 flex items-center" ref={seekBarRef}>
                            {/* Track bg */}
                            <div className="absolute inset-0 h-1.5 bg-card/8 rounded-full" />
                        ...
                        <span className="text-xs font-mono text-muted-foreground w-10 text-left shrink-0 tabular-nums">
                    ...
                    <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 border ${
                                    isLooping
                                        ...
                                        : 'bg-transparent border-white/10 text-muted-foreground hover:text-muted-foreground hover:border-white/20'
                                }`}
                            >
                        ...
                        <div className="flex items-center gap-3 justify-center w-1/3">
                            <button
                                ...
                                disabled={!isAudioReady || hasLoadError}
                                className="p-2 text-muted-foreground hover:text-primary-foreground transition-colors disabled:opacity-30"
                            >
                        ...
                        <div className="flex items-center justify-end w-1/3">
                            <div className="flex items-center gap-1.5 bg-card/5 border border-white/8 rounded-xl px-3 py-1.5">
                                <span className="text-xs text-muted-foreground font-mono mr-1">BPM</span>

                                <button
                                    onClick={() => handleTempoChange(tempo - 5)}
                                    disabled={!isAudioReady || tempo <= 50}
                                    className="w-6 h-6 rounded-md flex items-center justify-center text-muted-foreground hover:text-primary-foreground hover:bg-card/10 transition-all disabled:opacity-30"
                                >
                                    <ChevronDown className="w-3.5 h-3.5" />
                                </button>

                                <span className={`text-sm font-mono font-semibold tabular-nums w-8 text-center transition-colors ${
                                    tempo !== 100 ? 'text-amber-400' : 'text-primary-foreground'
                                }`}>
                                    {tempo}
                                </span>
                                ...
                                {tempo !== 100 && (
                                    <button
                                        ...
                                        className="w-5 h-5 rounded flex items-center justify-center text-muted-foreground hover:text-amber-400 transition-colors ml-0.5"
                                        title="Reset tempo"
                                    >
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary/10 border border-primary/20 text-primary text-xs font-mono hover:bg-primary/20 transition-colors"
                                >
                                    <span className="font-bold">A</span>
                                    <span className="text-primary/70">{formatTime(loopStart)}</span>
                                </button>
                                <button
                                    onClick={() => setLoopEnd(Math.min(maxDuration, playbackTime))}
                                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-xs font-mono hover:bg-destructive/10 transition-colors"
                                >
                                    <span className="font-bold">B</span>
                                    <span className="text-destructive/70">{formatTime(loopEnd)}</span>
                                </button>
                            </div>
                        </div>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    {/* Error badge */}
                                    {stem.hasLoadError && (
                                        <span className="px-2 py-0.5 text-xs font-mono text-destructive bg-destructive/10 border border-destructive/20 rounded-md shrink-0">
                                            ERR
                                        </span>
                                    )}
                                        ...
                                        <button
                                            ...
                                            className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${
                                                stem.isMuted
                                                    ...
                                                    : 'bg-card/5 text-muted-foreground hover:text-primary-foreground border border-white/5 hover:border-white/15'
                                            } disabled:opacity-30`}
                                        >
                ...
                {/* Footer hint */}
                <p className="text-center text-xs text-muted-foreground mt-8 font-mono tracking-wider">
